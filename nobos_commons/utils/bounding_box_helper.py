...
def get_human_bounding_box_3D_from_joints(joints: SkeletonJointsBase, max_x_val: int = sys.maxsize, max_y_val: int = sys.maxsize,
    ...
    return BoundingBox3D(top_left=Coord2D(x=int(min_x), y=int(min_y)),
                       bottom_right=Coord2D(x=int(max_x), y=int(max_y)), label="person")
