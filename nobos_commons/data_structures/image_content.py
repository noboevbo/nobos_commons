...
class ImageContent(object):
    __slots__ = ['img_path', 'human_content', 'object_bounding_boxes']

    def __init__(self,
                 human_content: ImageContentHumans = [],
                 object_bounding_boxes: List[BoundingBox] = [],
                 img_path: str = None,
                 ):
        """
...
class ImageContentBuffer(object):
    ...
    def add(self, image_content: ImageContent):
        ...
        updated_keys = []
        if image_content.human_content is not None:
            for human in image_content.human_content.humans:
    ...
    def get_last_human_content(self):
        if len(self.__image_contents) == 0:
            return None
        return self.__image_contents[-1].human_content
