...
def get_human_pose_image(original_img: np.ndarray, human_data: ImageContentHumans, limb_colors: List[Color], joint_colors: List[Color]):
...
def visualize_limb(original_img: np.ndarray, limb: Limb2D, limb_color: Color, stickwidth: int = 4, write_in_original_image: bool = False):
...
def visualize_straying_joints(original_img: np.ndarray, straying_joint_dict: Dict[int, List[Joint2D]], joint_colors: List[Color], write_in_original_image: bool = False):
    img = original_img if write_in_original_image else original_img.copy()
...
def visualize_joints(original_img: np.ndarray, joints: List[Joint2D], color: Color, write_in_original_image: bool = False):
    img = original_img if write_in_original_image else original_img.copy()
    for joint in joints:
        cv2.circle(img, tuple(joint.coordinates), 5, color.tuple_bgr, thickness=-1)
    return img